...
class QuestionService:
    ...
    def create_question(self, question_in: QuestionCreate, user_id: uuid.UUID, role: UserRole) -> Question:
        ...
        try:
            # Check for duplicate order_number in same exam
            existing_order = self.db.query(Question).filter(
                Question.exam_id == question_in.exam_id,
                Question.order_number == question_in.order_number,
                Question.is_deleted == False
            ).first()
            if existing_order:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Question with this order number already exists")
    ...
    def update_question(self, question_id: uuid.UUID, question_in: QuestionUpdate, user_id: uuid.UUID, role: UserRole) -> Question:
        ...
        if 'order_number' in update_data and update_data['order_number'] != question.order_number:
            existing_order = self.db.query(Question).filter(
                Question.exam_id == question.exam_id,
                Question.order_number == update_data['order_number'],
                Question.is_deleted == False,
    ...
    def list_questions_for_exam(self, exam_id: uuid.UUID, user_id: uuid.UUID, role: UserRole, skip: int = 0, limit: int = 100) -> tuple[List[Question], int]:
        # For students, they might only see questions when attempt starts (handled elsewhere)
        # For this admin/faculty endpoint:
        self._verify_exam_access(exam_id, user_id, role)
        
        query = self.db.query(Question).filter(Question.exam_id == exam_id, Question.is_deleted == False).order_by(Question.order_number)
        total = query.count()
    ...
    def duplicate_question(self, question_id: uuid.UUID, user_id: uuid.UUID, role: UserRole) -> Question:
        ...
        max_order = self.db.query(Question).filter(Question.exam_id == original.exam_id).order_by(Question.order_number.desc()).first()
        new_order = max_order.order_number + 1 if max_order else 1
    ...
    def bulk_create_questions(self, bulk_in: BulkQuestionCreate, user_id: uuid.UUID, role: UserRole):
        ...
        for i, q_in in enumerate(bulk_in.questions):
            try:
                # Basic create question logic without committing per row
                db_question = Question(
    ...
    def reorder_questions(self, exam_id: uuid.UUID, reorder_request: QuestionReorderRequest, user_id: uuid.UUID, role: UserRole) -> None:
        ...
        try:
            ...
            self.db.rollback()
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Database integrity error during reorder (check for duplicate order_numbers if constraints exist)")
